...
class AttributeGroupUpdater extends AkeneoAttributeGroupUpdater
{
    ...
    protected function setData($attributeGroup, $field, $data): void
    {
        if ('permission[allowed_to_edit]' === $field) {
            $this->setAccesses($attributeGroup, explode(',', $data), AttributeGroupAccess::EDIT_LEVEL);
        } elseif ('permission[allowed_to_view]' === $field) {
            $this->setAccesses($attributeGroup, explode(',', $data), AttributeGroupAccess::VIEW_LEVEL);
        } elseif ('permission[allowed_to_own]' === $field) {
            $this->setAccesses($attributeGroup, explode(',', $data), AttributeGroupAccess::OWN_LEVEL);
        } else {
            parent::setData($attributeGroup, $field, $data);
        }
    }

    private function setAccesses(AttributeGroupInterface $attributeGroup, array $ids, string $level): void
    {
        $groups = $this->userGroupRepository->findBy(['id' => $ids]);

        foreach ($groups as $group) {
            /** @var Group $group */
            if (!$attributeGroup->checkIfAccessExists($group, $level)) {
