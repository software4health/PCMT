...
: "${FTP_PUT_CREDS:=/run/secrets/ftp-put-creds}"
error_when_unreadable "$FTP_PUT_CREDS"

source "$FTP_PUT_CREDS"
...
: "${SFTP_LOCAL_DIR:=/downloads/work}"
error_when_unreadable "$SFTP_LOCAL_DIR"
: "${SFTP_LOCAL_ARCHIVE_DIR:=/downloads/done}"
error_when_unreadable "$SFTP_LOCAL_ARCHIVE_DIR"
